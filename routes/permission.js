...
permissionRouter.get(
  "/readgrant/fetch:reader",
  cors(),
  authenticate.verifyUser,
  (req, res, next) => {
    // fetch reader's public key
    User.findById(req.params.reader, (err, usr) => {
      if (err) {
        res.sendStatus(404);
        return;
      }
...
permissionRouter.post(
  "/readgrant/send:reader",
  cors(),
  authenticate.verifyUser,
  (req, res, next) => {
    // receive location hash encrypted by reader's public key
